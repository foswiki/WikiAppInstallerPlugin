...
sub TRYME {
    ...
    my $reqd_extensions = Foswiki::Func::getPreferencesValue('REQUIRED_EXTENSIONS');
    if ($reqd_extensions){
		...
		if (scalar @extensions gt 0 ){
    		foreach my $extension(@extensions){
				...
				#	If a Contrib/AddOn is referenced check that there is a topic in the %SYSTEMWEB% with the same name
    				...
    				unless ( Foswiki::Func::topicExists($Foswiki::cfg{SystemWebName}, $extension ) ){
...
sub doTRYME {
    ...
    unless (Foswiki::Func::webExists($targetWeb)) {
        ...
        Foswiki::Meta->new( $session, $targetWeb )->populateNewWeb('_empty', {});

		# Add a default WebHome topic which will list all applications in this new web
		($meta, $text) = ('', '');
		$text .= '---+!! Welcome to your personal playpen web for testing Web Apps';
		...
		$text .= '%SEARCH{"preferences[name=\'APP_MASTER_TOPIC\'].value=\'true\'" excludetopic="WebPreferences,WebHome" type="query" nonoise="on" format="   * [[$topic]]"}%';
    ...
    my $comment_pref = Foswiki::Func::getPreferencesValue('COMMENT_TEMPLATE_TOPIC');
    if ($comment_pref){
    	my @comment_topics = split(/, */, $comment_pref);
        if (Foswiki::Func::topicExists($targetWeb, $user_comments_template)){
		    my ($meta, $text) = Foswiki::Func::readTopic($targetWeb, $user_comments_template);
			$text .= "\n%TMPL:INCLUDE{\"$comment_topics[0]\"}%\n";
        	Foswiki::Func::saveTopic($targetWeb, $user_comments_template, $meta, $text,
            { dontlog => 1, minor => 1 });
		}else{
    ...
    foreach my $topic (@topics) {
		...
		if ( $app_landing_topic eq $topic ){
			$meta->putAll( 'PREFERENCE',
     			{ name => 'BROADCASTMESSAGE', value =>"You are using a test version of this wiki application in your personal sandbox web. See [[$original_web.$original_topic]] for further information, including details on how to download this Wiki App for use in your local Foswiki installation." },
		...
		if (scalar @attachment_list gt 0 ){
			foreach my $attachment(@attachment_list){
				try {
   					Foswiki::Func::copyAttachment( $web, $topic, $attachment, $targetWeb, $topic );
	...
	Foswiki::Func::redirectCgiQuery( undef, Foswiki::Func::getScriptUrl($targetWeb, $app_landing_topic, 'view'), 0);
}
...
sub PACKAGEME {
	...
	my ($meta, $text) = Foswiki::Func::readTopic($web, $topic);
	my @manifest_details = split (/, */, Foswiki::Func::getPreferencesValue('MANIFEST'));	
	my $return_string;
